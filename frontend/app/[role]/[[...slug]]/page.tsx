...
function Timeline({ steps }: { steps: { name: string; status: string; action?: string | null }[] }) {
  return (
    <ol className="space-y-0">
      {steps.map((x, i) => (
        <li key={x.name} className="relative flex gap-4 pb-5">
          <span
            className={`z-10 mt-1 grid h-6 w-6 place-items-center rounded-full text-xs ${
          ...
          <div>
            <p className="font-bold text-navy">{x.name}</p>
      ...
      <section className="mt-7 grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        {d.journey.length > 0 && (
          <div className="card">
            ...
            <Timeline steps={d.journey.map((j) => ({ name: j.label, status: j.status === 'done' ? 'completed' : j.status === 'current' ? 'in_progress' : 'not_started' }))} />
          </div>
        )}
    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <div className="card">
          ...
          <Timeline steps={sorted.map((s) => ({ name: s.step_name, status: s.status, action: s.next_action }))} />
        </div>
...
function Appointments() {
  ...
  return (
    <>
      <Title eyebrow="Appointments" title="Your appointments" copy="View existing appointments or book a new one." />
      {confirmed && (
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2">
          {existing.map((a) => (
            <div key={a.id} className="card">
              ...
              <p className="mt-1 text-sm text-slate-600">Status: {a.status} · {a.booking_method.replace('_', ' ')}</p>
        ...
        <div className="card max-w-2xl">
          ...
          <fieldset className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            {slots.filter((s) => s.booked_count < s.capacity).map((s) => (
              <label
                key={s.id}
                className={`cursor-pointer rounded-xl border p-4 font-bold ${selectedSlot?.id === s.id ? 'border-teal bg-mint text-teal' : ''}`}
              >
...
function Grievances() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.grievances.create({ category: 'general', subject, description: desc });
      setSubject('');
      setDesc('');
      api.grievances.list().then(setList);
    } catch (e: any) {
  ...
  return (
    <>
      <Title eyebrow="Support and resolution" title="My grievances" copy="Track existing grievances or file a new one." />
      {list?.map((g) => (
        <div key={g.id} className="card mb-4 max-w-3xl">
          <div className="flex items-center gap-3">
            <span className={`pill ${g.status === 'acknowledged' || g.status === 'action_taken' ? 'bg-mint text-teal' : 'bg-amber-100 text-amber-900'}`}>
              {g.status.replace('_', ' ').toUpperCase()}
            </span>
            <span className="text-sm text-slate-500">{g.grievance_number}</span>
          </div>
          <h2 className="mt-3 text-xl font-black text-navy">{g.subject}</h2>
      ...
      <form onSubmit={submit} className="card mt-6 max-w-3xl">
        <h2 className="font-black text-navy">Lodge a new grievance</h2>
        <label className="label mt-4">
          Subject
          <input required value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-2 w-full rounded-xl border p-3" />
        </label>
        <label className="label mt-4">
          Description
          <textarea required value={desc} onChange={(e) => setDesc(e.target.value)} className="mt-2 min-h-24 w-full rounded-xl border p-3" />
        </label>
...
export default function PortalPage() {
  ...
  if (role === 'citizen') {
    ...
    else if (page === 'grievances') content = <Grievances />;
